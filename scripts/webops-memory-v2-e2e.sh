...
RUN_ID="${WEBOPS_MEMORY_V2_E2E_RUN_ID:-latest}"
ARTIFACT_ROOT="${WEBOPS_MEMORY_V2_E2E_ARTIFACT_DIR:-$ROOT_DIR/artifacts/webops-memory-v2/$RUN_ID}"

POSTGRES_CONTAINER_NAME="${WEBOPS_MEMORY_V2_POSTGRES_CONTAINER:-page-agent-webops-memory-v2-postgres-e2e}"
POSTGRES_PORT="${WEBOPS_MEMORY_V2_POSTGRES_PORT:-54332}"
POSTGRES_DB="${WEBOPS_MEMORY_V2_POSTGRES_DB:-page_agent_webops_memory_v2_e2e}"
POSTGRES_USER="${WEBOPS_MEMORY_V2_POSTGRES_USER:-page_agent}"
POSTGRES_PASSWORD="${WEBOPS_MEMORY_V2_POSTGRES_PASSWORD:-page_agent_dev}"
WORKFLOW_BACKEND_ADDR="${WEBOPS_MEMORY_V2_BACKEND_ADDR:-127.0.0.1:38432}"

BACKEND_PID=""

if [[ "${KEEP_WEBOPS_MEMORY_V2_E2E_ARTIFACTS:-0}" != "1" ]]; then
	rm -rf "$ARTIFACT_ROOT"
fi
...
cleanup() {
	...
	if [[ "${KEEP_WEBOPS_MEMORY_V2_E2E:-0}" != "1" ]]; then
...
cat >"$ARTIFACT_ROOT/e2e-env.json" <<EOF
{
...
(
	...
	NO_PROXY="127.0.0.1,localhost" \
		...
		WEBOPS_MEMORY_V2_ARTIFACT_DIR="$ARTIFACT_ROOT" \
		WEBOPS_MEMORY_V2_POSTGRES_CONTAINER="$POSTGRES_CONTAINER_NAME" \
		WEBOPS_MEMORY_V2_POSTGRES_DB="$POSTGRES_DB" \
		WEBOPS_MEMORY_V2_POSTGRES_USER="$POSTGRES_USER" \
		node scripts/webops-memory-v2-e2e.mjs
)

echo "WebOps memory V2 E2E artifacts: $ARTIFACT_ROOT"
