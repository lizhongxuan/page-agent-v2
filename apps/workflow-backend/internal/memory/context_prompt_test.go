...
func TestFormatMemoryContextPromptUsesWebOpsMemory(t *testing.T) {
	response := MemoryContextResponse{
		...
		SiteTaskGuides: []SiteTaskGuideHint{
			{ID: "guide_service_status", WhenToUse: "查询服务状态。", Steps: []string{"使用服务名称搜索框。", "打开详情页。"}, AbandonRules: []string{"当前页没有服务搜索框时放弃。"}},
		},
		SiteManualKnowledge: []SiteManualKnowledgeHint{
			{ID: "manual_service", Title: "服务管理手册", Summary: "服务管理页可通过服务名称搜索框定位服务。", SourceRefs: []string{"服务管理手册#搜索"}},
		},
	...
	if !strings.Contains(prompt, "<webops_memory>") {
		t.Fatalf("unexpected prompt wrapper: %s", prompt)
	}
	for _, expected := range []string{"<current_page", "<navigation_hints>", "<site_task_guides>", "<site_manual_knowledge>", "<failure_warnings>"} {
		if !strings.Contains(prompt, expected) {
			t.Fatalf("expected %s in prompt: %s", expected, prompt)
		}
	}
	if strings.Contains(prompt, "<experience_hints>") {
		t.Fatalf("legacy experience hints should not be formatted: %s", prompt)
	}
}
...
func TestFormatMemoryContextPromptTruncatesAndRejectsSensitiveText(t *testing.T) {
	response := MemoryContextResponse{
		SiteManualKnowledge: []SiteManualKnowledgeHint{
			{Title: "token=secret-value", Summary: strings.Repeat("a", MaxSummaryChars+20)},
		},
	}
	prompt := FormatMemoryContextPrompt(response)
