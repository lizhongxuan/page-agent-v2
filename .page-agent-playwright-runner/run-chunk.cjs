...
function targetSelectorText(target) {
	if (!target) return ''
	return [target.strategy, target.value, target.role, target.name]
		.filter(Boolean)
		.map(render)
		.join(' ')
}
...
async function controlVisible(page, control) {
	...
	try {
		if (control.role && control.name) {
			return await page
				.getByRole(control.role, { name: control.name })
				.first()
...
function pageStateMatches(expectedPageState, observation, chunk) {
	...
	if (expected === 'github_repo_home') {
		...
		if (parsed?.hostname === 'github.com' && segments.length >= 2 && !path.includes('/issues'))
			return true
	}
	if (
		expected.includes('github_issues') &&
		(path.includes('/issues') || haystack.includes('issues'))
	) {
		return true
	}
